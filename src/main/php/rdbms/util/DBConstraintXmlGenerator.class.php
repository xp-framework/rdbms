...
class DBConstraintXmlGenerator extends \lang\Object implements Traceable {
  ...
  public static function createFromDatabase($adapter, $database) {
    ...
    $g->doc->root()->addChild(new \xml\Node('database', null, array(
      'database' => $database
    )));
  ...
  public function getTree() {
    foreach ($this->tables as $t) {
      $constKeyList= array();
      $tn= $this->doc->root()->nodeAt(0)->addChild(new \xml\Node('table', null, array(
        'name' => $t->name,
      )));
      ...
      if ($constraint= $t->getFirstForeignKeyConstraint()) do {
        ...
        $cn= $tn->addChild(new \xml\Node('constraint', null, array(
